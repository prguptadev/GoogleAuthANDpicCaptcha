...
$("#btnRegister").click(function () {
    $.post("/user/register/" + $("#login").val() + "/" + $("#password").val(), function (data, status) {
        if (status == 'success') {
           //old// $("#tokenQr").attr("src", "https://zxing.org/w/chart?cht=qr&chs=250x250&chld=M&choe=UTF-8&chl=otpauth://totp/"+$("#login").val()+"?secret=" + data + "&issuer=localhost.com");
          //  $("#tokenQr").attr("src", "https://zxing.org/w/chart?cht=qr&chs=250x250&chld=M&choe=UTF-8&chl=otpauth://totp/2FaExample.com?secret=" + data + "&issuer=2FaExample");
            $("#tokenQr").attr("src", "http://localhost:8080/authenticate/qrcode/"+$("#login").val()+"/"+data);
